    ...
    public class Avoidance : AIMovement
    {
        ...
        public override SteeringOutput GetSteering(Task_7.AI.AIAgent agent)
        {
            ...
            foreach (GameObject obstacle in agent.Obstacles)
            {
                ...
                else if (distanceSquared > squaredRadius && agent.CurrentState == Task_7.AI.AIAgent.ShipState.Avoiding)
                {
                    if (agent.CompareTag("PirateShip"))
                    {
                        agent.SetState(Task_7.AI.AIAgent.ShipState.Wandering);
                    }
                    else if (agent.CompareTag("TradeShip"))
                    {
                        agent.SetState(Task_7.AI.AIAgent.ShipState.Navigating);
                    }
                }
            }
